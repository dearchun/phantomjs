...
// DefaultProcess returns  default process.
func DefaultProcess() *Process {
	return NewProcess(DefaultPort)
}
...
// GetResourceUrls resource request urls
func (p *WebPage) GetResourceUrls() []string{
	var resp struct {
		ReturnValue []string `json:"value"`
	}
	p.ref.process.doJSON("POST", "/webpage/ResourceUrls", map[string]interface{}{"ref": p.ref.id}, &resp)
...
server.listen(system.env["PORT"], function(request, response) {
	try {
		switch (request.url) {
			...
			case '/webpage/ResourceUrls': return handleResourceUrls(request, response);
...
function handleWebpageCreate(request, response) {
	...
	// keep resource request urls
	page.onResourceRequested = function(requestData, networkRequest) {
		currenturls.push(requestData.url);
  		resourceUrl[ref.id] = currenturls;
	};
...
function handleResourceUrls(request, response) {
	var msg = JSON.parse(request.post);
	res = resourceUrl[msg.ref];
	response.write(JSON.stringify({value: res}));
	currenturls = new Array();
	response.closeGracefully();
}
